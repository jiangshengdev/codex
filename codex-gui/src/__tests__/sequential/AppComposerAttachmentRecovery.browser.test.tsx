...
test.each([
  ...
  { name: "b.heic", type: "image/heic" },
])(
  "Pending editing restores an attachment and waits for $name before saving",
  async ({ name, type }) => {
    ...
    await attachmentFileInput(screen.getByRole("dialog").element()).upload(
      new File(["B"], name, { type }),
    );
    await expect.element(pendingEditor.getByText(name, { exact: true })).toBeVisible();
    ...
    expect(startTurnParamsAt(startTurn, 0).input).toEqual([
      {
        ...
        text_elements: [
          ...
          { byteRange: { start: 11, end: 21 }, placeholder: name },
        ],
      },
    ]);
    expect(steerTurn).not.toHaveBeenCalled();
