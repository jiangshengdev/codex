...
import {
  ...
  useChatUiSession,
} from "@/features/chatUiSession/ChatUiSessionProvider";
...
function StickyBottomSurface({ contentHeight }: { contentHeight: number }) {
  ...
  return (
    <>
      ...
      >
        <button
          onClick={() => {
            captureAndUnmount(true);
          }}
          type="button"
        >
          Capture sticky and unmount
        </button>
        <button
          onClick={() => {
            captureAndUnmount(false);
          }}
          type="button"
        >
          Capture position and unmount
        </button>
        <button
          onClick={() => {
      ...
      {chatMounted ? (
        <StickyBottomSurface contentHeight={contentHeight} />
      ) : (
...
test("keeps the chat session and projection current while the chat route DOM is unmounted", async () => {
  ...
  await expect.element(screen.getByTestId("settings-route")).toBeVisible();
...
test("the real sticky-bottom hook restores a remounted chat to the latest bottom", async () => {
  ...
  await screen.getByRole("button", { name: "Capture sticky and unmount" }).click();
  await expect
    .element(screen.getByRole("region", { name: "Test transcript surface" }))
    .not.toBeInTheDocument();
...
test("the real sticky-bottom hook restores a saved position once without later jumping", async () => {
  ...
  await screen.getByRole("button", { name: "Capture position and unmount" }).click();
  await screen.getByRole("button", { name: "Add transcript content" }).click();
  await screen.getByRole("button", { name: "Remount chat" }).click();
