...
export function LanguageAutocomplete() {
  ...
  const clearSearch = t({
    comment: "Accessible label for the button that clears the language search input",
    message: "Clear search",
  });

  const setHeadingRef = useCallback((heading: HTMLHeadingElement | null): void => {
    if (headingFocusFrameRef.current != null) {
      cancelAnimationFrame(headingFocusFrameRef.current);
      headingFocusFrameRef.current = null;
    }
    if (heading != null) {
      headingFocusFrameRef.current = requestAnimationFrame(() => {
        headingFocusFrameRef.current = null;
  ...
  const onPreferenceChange = async (value: Key | Key[] | null): Promise<void> => {
    ...
    try {
      ...
      // The locale runtime publishes the failure. Keep this selection surface open so the user
      // can retry without losing the current query.
    } finally {
          ...
          <Autocomplete.Filter filter={contains} inputValue={query} onInputChange={setQuery}>
            ...
            >
              <SearchField.Group>
                ...
                <SearchField.ClearButton aria-label={clearSearch} />
              </SearchField.Group>
            </SearchField>
            <ListBox
              renderEmptyState={() => (
