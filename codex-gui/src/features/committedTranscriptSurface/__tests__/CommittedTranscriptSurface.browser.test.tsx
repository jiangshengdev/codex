...
const localizedTranscriptShellExpectations = [
  {
    locale: "en",
...
const dynamicTranscriptText = "DYNAMIC transcript content · 保持原样";
const dynamicThreadId = "dynamic-thread-id";

const semanticActivityTurnId = "semantic-activity-turn";
const semanticActivityChunkId = `${semanticActivityTurnId}:chunk:0`;
const semanticSpawnEntryId = "semantic-spawn-activity";
const semanticStartedEntryId = "semantic-started-activity";
const semanticUserPrompt = "USER prompt 原文";
const semanticActivityPrompt = "ACTIVITY prompt 原文";
const semanticAgentPath = "/root/semantic-reviewer-原文";
const semanticThreadId = "thread-semantic-原文";
const semanticModel = "gpt-5.6-semantic-原文";
const semanticEffort = "xhigh-原文";
const semanticMessage = "agent result message 原文";

function LocaleSwitchControl() {
  const { setPreference } = useLocaleRuntime();
  return (
...
const createSemanticActivityState = (): TranscriptState => ({
  ...createEmptyTranscriptState(),
  threadId: "semantic-transcript-thread",
  turnIds: [semanticActivityTurnId],
  turnsById: {
    [semanticActivityTurnId]: {
      id: semanticActivityTurnId,
      status: "completed",
      leadingPromptEntryId: "semantic-user-prompt",
      middleChunkIds: [semanticActivityChunkId],
      middleEntryCount: 2,
      finalAssistantEntryIds: [],
    },
  },
  chunksById: {
    [semanticActivityChunkId]: {
      id: semanticActivityChunkId,
      turnId: semanticActivityTurnId,
      entryIds: [semanticStartedEntryId, semanticSpawnEntryId],
      revision: 11,
    },
  },
  entriesById: {
    "semantic-user-prompt": {
      type: "message",
      id: "semantic-user-prompt",
      turnId: semanticActivityTurnId,
      role: "user",
      source: semanticUserPrompt,
      sourceKind: "plainText",
      phase: null,
      revision: 3,
    },
    [semanticStartedEntryId]: {
      type: "activity",
      id: semanticStartedEntryId,
      turnId: semanticActivityTurnId,
      copy: { kind: "agentStarted", agentPath: semanticAgentPath },
      details: [],
      revision: 5,
    },
    [semanticSpawnEntryId]: {
      type: "activity",
      id: semanticSpawnEntryId,
      turnId: semanticActivityTurnId,
      copy: {
        kind: "agentSpawned",
        receiver: semanticThreadId,
        model: semanticModel,
        reasoningEffort: semanticEffort,
      },
      details: [
        { kind: "raw", text: semanticActivityPrompt },
        {
          kind: "copy",
          copy: {
            kind: "agentStatus",
  ...
  entryChunkById: {
    [semanticStartedEntryId]: semanticActivityChunkId,
    [semanticSpawnEntryId]: semanticActivityChunkId,
  },
});

const createLocalizedTranscriptState = () => {
...
test.each(localizedTranscriptShellExpectations)(
  "renders the empty transcript shell in $locale",
  async ({ emptyState, locale, regionLabel }) => {
    const screen = await renderWithProviders(<CommittedTranscriptSurface />, { locale });
...
test.each(localizedTranscriptShellExpectations)(
  ...
  async (expectation) => {
    ...
    expect(store.getState().transcriptState.turnsById["dynamic-turn-failed"]?.status).toBe("failed");
  },
);
...
test("switches semantic activity copy without rewriting transcript entries or chunks", async () => {
  const { store, ...screen } = await renderWithProviders(
    ...
    {
      locale: "en",
      preloadedState: { transcriptState: createSemanticActivityState() },
    },
  );
  const entryBefore = selectTranscriptEntry(store.getState(), semanticSpawnEntryId);
  const chunkBefore = selectTranscriptChunk(store.getState(), semanticActivityChunkId);
  const rawChunkBefore = store.getState().transcriptState.chunksById[semanticActivityChunkId];

  expect(entryBefore?.revision).toBe(7);
  expect(chunkBefore?.revision).toBe(11);
  expect(rawChunkBefore?.revision).toBe(11);

  const startedEnglish = screen.getByRole("article", {
    name: `Started ${semanticAgentPath}`,
  });
  const spawnedEnglish = screen.getByRole("article", {
    name: `Spawned ${semanticThreadId} (${semanticModel} ${semanticEffort})`,
  });
  await expect.element(startedEnglish).toBeVisible();
  await expect.element(spawnedEnglish).toBeVisible();
  await expect.element(spawnedEnglish).toHaveTextContent(semanticThreadId);
  await expect.element(spawnedEnglish).toHaveTextContent(semanticModel);
  await expect.element(spawnedEnglish).toHaveTextContent(semanticEffort);
  await expect.element(spawnedEnglish.getByText(semanticActivityPrompt, { exact: true })).toBeVisible();
  await expect
    .element(
      spawnedEnglish.getByText(`${semanticThreadId}: Completed - ${semanticMessage}`, {
        exact: true,
      }),
    )
    .toBeVisible();
  await expect.element(screen.getByText(semanticUserPrompt, { exact: true })).toBeVisible();

  await screen.getByRole("button", { name: "Switch activity locale" }).click();

  const startedChinese = screen.getByRole("article", {
    name: `已启动 ${semanticAgentPath}`,
  });
  const spawnedChinese = screen.getByRole("article", {
    name: `已启动 ${semanticThreadId}（${semanticModel} ${semanticEffort}）`,
  });
  await expect.element(startedChinese).toBeVisible();
  await expect.element(spawnedChinese).toBeVisible();
  await expect.element(startedEnglish).not.toBeInTheDocument();
  await expect.element(spawnedEnglish).not.toBeInTheDocument();
  await expect.element(spawnedChinese).toHaveTextContent(semanticThreadId);
  await expect.element(spawnedChinese).toHaveTextContent(semanticModel);
  await expect.element(spawnedChinese).toHaveTextContent(semanticEffort);
  await expect.element(spawnedChinese.getByText(semanticActivityPrompt, { exact: true })).toBeVisible();
  await expect
    .element(
      spawnedChinese.getByText(`${semanticThreadId}：已完成 - ${semanticMessage}`, {
        exact: true,
      }),
    )
    .toBeVisible();
  ...
  const entryAfter = selectTranscriptEntry(store.getState(), semanticSpawnEntryId);
  const chunkAfter = selectTranscriptChunk(store.getState(), semanticActivityChunkId);
  const rawChunkAfter = store.getState().transcriptState.chunksById[semanticActivityChunkId];
  expect(entryAfter).toBe(entryBefore);
  expect(entryAfter?.revision).toBe(7);
  expect(chunkAfter).toBe(chunkBefore);
  expect(chunkAfter?.revision).toBe(11);
  expect(rawChunkAfter).toBe(rawChunkBefore);
  expect(rawChunkAfter?.revision).toBe(11);
});
