...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn reject_unsupported_images_preserves_settings_before_start() -> Result<()> {
    let server = start_mock_server().await;
    ...
    let text_model = test_model_info("preview-text", "text", "text", vec![InputModality::Text]);
    let mut builder = test_codex()
    ...
    for image in [
        ...
        UserInput::LocalImage {
            path: std::path::PathBuf::from("/nonexistent/image.png"),
            detail: None,
        },
    ] {
        let mut request = read_only_user_turn(&test, vec![image], "preview-text".to_string());
        request.reject_unsupported_images = true;
        let error = test
            .codex
            .start_or_steer_turn(request)
            .await
            .expect_err("unsupported images must be rejected");
        assert!(
            matches!(error.details(), codex_protocol::error::CodexErrorDetails::InvalidRequest(message) if message.contains("does not support image input")),
            "{error:?}"
        );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn reject_unsupported_images_uses_live_model_when_steering() -> Result<()> {
    ...
    let (server, _) = start_streaming_sse_server(vec![vec![
        StreamingSseChunk {
            gate: None,
            body: sse(vec![ev_response_created("resp-live")]),
        },
        StreamingSseChunk {
            gate: Some(gate),
            body: sse(vec![ev_completed("resp-live")]),
        },
    ]])
    .await;
    let image_model = test_model_info(
    ...
    let test = test_codex()
        ...
        .with_config(move |config| {
            config
                ...
                .expect("enable live model changes for this scenario");
            config.model_catalog = Some(ModelsResponse {
                models: vec![image_model, text_model],
            });
        })
    ...
    let started = test
        .codex
        .start_or_steer_turn(TurnInputRequest::user_input(vec![UserInput::Text {
            text: "hold this response".to_string(),
            text_elements: Vec::new(),
        }]))
        .await?;
    ...
    let original = test.codex.thread_settings_snapshot().await;
    // Both the initial-turn model and future thread default still support images.
    // Only the active turn's updated model is text-only.
    for steer_only in [false, true] {
        let mut request = TurnInputRequest::user_input(vec![UserInput::LocalImage {
            path: std::path::PathBuf::from("/nonexistent/image.png"),
            detail: None,
        }]);
        request.reject_unsupported_images = true;
        let error = if steer_only {
            test.codex
                .steer_turn(request, turn_id.clone())
                .await
                .expect_err("live text model must reject images")
        } else {
            test.codex
                .start_or_steer_turn(request)
                .await
                .expect_err("turn/start steering must use the live model")
        };
        assert!(
            matches!(error.details(), codex_protocol::error::CodexErrorDetails::InvalidRequest(message) if message.contains("preview-text") && message.contains("does not support image input")),
            "{error:?}"
        );
        assert_eq!(test.codex.thread_settings_snapshot().await, original);
    }
    release.send(()).expect("release held response");
    wait_for_event(&test.codex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
#[test_case(false, "preview-text"; "default false keeps text-only behavior")]
#[test_case(true, "preview-image"; "supported images accepted with opt-in")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn reject_unsupported_images_accepts_supported_or_default_input(
    reject_unsupported_images: bool,
    destination: &str,
) -> Result<()> {
    let server = start_mock_server().await;
    let image_model = test_model_info(
    ...
    let test = builder.build(&server).await?;
    let responses = mount_sse_once(&server, sse_completed("resp-image-admission")).await;
    let mut request = read_only_user_turn(&test, vec![UserInput::Image {
        image_url: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR4nGP4z8DwHwAFAAH/iZk9HQAAAABJRU5ErkJggg==".to_string(),
        detail: None,
    }], destination.to_string());
    request.reject_unsupported_images = reject_unsupported_images;
    ...
    assert_eq!(body["model"].as_str(), Some(destination));
    let has_image = body["input"].as_array().unwrap().iter().any(|item| {
        item["content"]
            .as_array()
            .is_some_and(|content| content.iter().any(|part| part["type"] == "input_image"))
    });
    assert_eq!(has_image, reject_unsupported_images);
    Ok(())
}
