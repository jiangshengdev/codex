    ...
    packageManager === "bun"
      ...
      : packageManager === "pnpm"
        ...
        : packageManager === "vite-plus"
          ? "vp install -g @jiangshengdev/codex@latest"
...
function isVitePlusOwnedCodexInstall(packagesDir) {
  ...
  try {
    const metadata = JSON.parse(
      readFileSync(
        path.join(packagesDir, "@jiangshengdev", "codex.json"),
        "utf8",
      ),
    );
    if (metadata.name !== "@jiangshengdev/codex") {
      return false;
    }

    // Vite+ records the active global installation in packages/@jiangshengdev/codex.json.
    ...
    const installDir = installId.startsWith("#")
      ? path.join(packagesDir, `@jiangshengdev/codex${installId}`)
      : path.join(packagesDir, "@jiangshengdev/codex", installId);
    for (const nodeModulesDir of [
      path.join(installDir, "lib", "node_modules"),
      path.join(installDir, "node_modules"),
    ]) {
      const packageRoot = path.join(nodeModulesDir, "@jiangshengdev", "codex");
      if (
