...
_PYTHON_RUNTIME_VERSION_PATTERN = re.compile(
    r"[0-9]+\.[0-9]+\.[0-9]+(?:a[0-9]+(?:\.post[0-9]+)?|\.dev[0-9]+)?"
)
_NORMALIZED_CODEX_VERSION_PATTERN = re.compile(
    r"[0-9]+(?:\.[0-9]+)*(?:(?:a|b|rc)[0-9]+(?:\.post[0-9]+)?|\.post[0-9]+|\.dev[0-9]+)?"
)
...
def resolve_python_runtime_release(python_version: str) -> tuple[str, str]:
    if _PYTHON_RUNTIME_VERSION_PATTERN.fullmatch(python_version) is None:
        raise RuntimeError(
            "Python runtime version must be stable, a numbered alpha, an "
            "alpha post-release, or a dev release, for example 0.136.0, "
            f"0.136.0a2, 0.136.0a2.post1, or 0.136.0.dev1; found {python_version}"
        )
...
def codex_release_version(version: str) -> str:
    ...
    cdx_release = re.fullmatch(r"([0-9]+(?:\.[0-9]+)*)\.dev([0-9]+)", normalized)
    if cdx_release is not None:
        base, number = cdx_release.groups()
        return f"{base}-cdx.{number}"
