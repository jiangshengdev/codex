...
规格、实施工单、正式调研结论、验收结果和完成说明使用 `jiangshengdev/codex` 仓库的 CNB Issues。
结论和证据保存在关联 Issue 中。CNB 是正式任务记录与状态的权威来源。
...
- 草稿、日志和待补录内容作为本地临时资料，不建立第二套任务追踪系统，不引入自动同步。
- 既有 `docs/superpowers/` 文件仅作为本地历史参考。不要在那里新建任务记录、迁移旧内容、同步到 CNB，或因工作流迁移删除这些资料；继续保持 Git ignore。
- 共享领域术语和长期架构决策沿用 `domain.md` 中的布局，覆盖 GUI 与 Rust 后端。

## CNB 不可用时

只有任务要求、验收标准及相关依赖信息完整可用且足以支持工作时，才继续已经授权的本地实现和验证。
缺少关键信息时，暂停受影响的工作并说明缺口。服务不可用不代表允许新领取任务，也不能证明阻塞事项已经完成。

将待发布结果保存在本地临时资料中，附上 Issue 引用、证据和拟补录内容。
分别报告本地进展与 CNB 状态；读回核验成功前，不得宣称远端已更新。
这些资料仅用于待补录交接，不构成第二套任务状态系统或同步服务。

恢复后检查认证，在进行已授权更新前，重新读取完整 Issue、评论、标签和相关阻塞事项。
先核对较新的要求或冲突修改；若这些变化使原工作前提失效，暂停受影响工作。
补录时保留他人的较新内容，随后读回验证。写入结果不确定时，先检查是否已经生效，不要盲目重试造成重复记录。

这些规则保留本地修改、外部写入、提交和发布的现有授权边界。允许本地继续不代表授权后续写入问题追踪器。
