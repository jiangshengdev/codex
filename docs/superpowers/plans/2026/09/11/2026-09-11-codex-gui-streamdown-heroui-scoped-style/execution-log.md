...
- 用户明确“允许这个任务输入”后，在其指定的新任务 Local 页面发送一条验收消息，要求仅输出 Markdown 代码、宽表格及公式，不调用工具或修改文件。发送前页面没有已提交消息；本次只发送一次。
- 390px 深色主题下逐秒采样真实响应：状态进入“进行中”，页面文字长度由 301 增长至 1904，Streamdown 元素由 0 增长至 56，过程中观察到 caret；后续状态回到“空闲”，文字长度 3287，两处 KaTeX、零 katex-error、零 data-sd-animate。采样期间及完成后 document scrollWidth 均为 390。此证据补齐真实输出增长到完成的生命周期；未逐帧检查所有中间状态或浅色流式过程。
- 完成后浅深主题 × 1440px/390px，分别对代码正文、宽表格容器、块公式执行真实鼠标横向 wheel。12 组场景均观察到局部 scrollLeft 在 0 与 200 之间改变，document scrollLeft 始终为 0，页面 scrollWidth 等于 viewport。
- 代码内容 scrollWidth=2353，容器宽度分别 684/306；表格内容 scrollWidth=2298，容器宽度分别 684/306；长公式 scrollWidth=1641，容器宽度分别 704/326。浅深表头仍分别为已验证的 surface-secondary 颜色。
- 390px 深色全屏宽表格：滚动容器宽 390、scrollWidth=2332，鼠标横向滚动至 200，页面横向位置为 0、宽度 390，退出按钮正常关闭。
- 新任务包含真实用户 card--secondary 和助手 card--default，深色背景分别为 surface-secondary 和 surface；没有完成思考条目，因此不补造该角色证据。
- 本次未发现新的样式问题，未修改产品代码。此前真实流式、长公式和宽表格滚动缺口已按上述明确范围补齐；完成思考角色矩阵、浅色真实流式过程及完整外部组件/全局工具类 Level 2 隔离矩阵仍未完整验证。此前范围外质量失败保持原结论，不能宣布全计划完全通过。
