...
不引入分页持久化、cursor 或新协议字段，只修正 successful canonical compaction event 的既有
持久化与历史重建。
...
修复顺序必须先让 Rust 历史重建保留 canonical completed identity，再让 GUI snapshot/live 走同一
completed boundary 分支。不得用 token、turn 数量、时间邻近、文本或其他启发式信号猜测压缩，也
不得用启发式去重掩盖 identity 缺口。
...
- 不在行为提交中混入 import、声明、函数、组件等纯顺序调整。若确需纯重排，停止并拆为新的
  独立任务，不顺手混入本计划提交；
- 最终 canonical completed 是新历史的唯一权威来源；只保留代码当前已有的旧 echo-only rollout
  重建能力。canonical 存在时 deprecated echo 只是配对兼容回声，不构成第二来源；不得新增临时
  双写、双读、compat path、adapter、计划外 fallback 或旧新权威来源并存。
...
- 当前分支和 HEAD 与 `dev @ c3005ac87c335d1a4cf05c0a144765c6dc45d1a3` 一致；若已漂移，
  先只读评估计划是否仍适用；
...
- 用户确认本计划后，只把计划状态从“待确认”改为“已确认”，记录确认日期与用户确认原文；
- 设计正文已经确认，不重写设计决策；
- 此任务只包含设计与计划文档，不包含代码。
...
git diff --cached
git commit -m 'docs(gui): design context compaction pagination'
```
...
- `ThreadHistoryBuilder` 以 canonical completed 为优先来源，用原始 `turnId + item.id` 重建
  compaction marker；
- canonical completed 后的配对 deprecated echo 只作为兼容回声消费，不追加第二个 marker；
...
- canonical completed + 配对 deprecated echo 只生成一个 marker；
...
```bash
git diff --check -- codex-rs/rollout/src/policy.rs codex-rs/rollout/src/policy_tests.rs codex-rs/app-server-protocol/src/protocol/thread_history.rs codex-rs/app-server/tests/suite/v2/compaction.rs
git add -- codex-rs/rollout/src/policy.rs codex-rs/rollout/src/policy_tests.rs codex-rs/app-server-protocol/src/protocol/thread_history.rs codex-rs/app-server/tests/suite/v2/compaction.rs
git diff --cached --check
git diff --cached --name-only
git diff --cached
...
正文引用不复制。覆盖固定落在 item policy、context pages、reconnect、replay dedup、selector cache
和 threadRuntime 测试；不为相同事实重复造测试。
...
- 四个提交的 subject 分别为：
  - `docs(gui): design context compaction pagination`
