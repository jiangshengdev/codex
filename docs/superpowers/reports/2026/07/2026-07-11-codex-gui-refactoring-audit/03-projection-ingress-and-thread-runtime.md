...
- **关联的既有报告、issue 或专项设计：** [live agent delta accumulation 计划](../../../../plans/2026/07/2026-07-09-codex-gui-live-agent-delta-accumulation.md) 曾有意保留单条 action 作为 compatibility baseline，并在 batch action 中优化同 item delta；该历史计划不是本条 Evidence owner。本 finding 不否定该设计，只记录 production handoff 完全转为 batch 后的局部清理机会；无直接关联 issue。
