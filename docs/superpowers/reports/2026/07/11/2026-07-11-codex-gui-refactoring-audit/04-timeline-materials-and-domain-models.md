...
| R04-TIMELINE-OWNER（依赖 `R04-SNAPSHOT,R04-LIVE`） | `complete` | production bridge 直接投递 runtime actions，transcript state 投影后由 UI selector 消费，完全绕过 timeline-material 管道。 | RA-04-001；审计时确认重构点，现已实施（B07），P2 | `GuiHostConnectionBridge.tsx:17-25,61-70,91-106`；`transcriptStateSlice.ts:2-8,82-98,137-143`；`CommittedTranscriptSurface.tsx:3-13,265-276` |
...
- **审计时 owner 与职责：** `snapshotReplay.ts` 从 runtime snapshot turns 构造 snapshot materials；`liveEventHandling.ts` 从 runtime event buffer 构造 live/subscription materials，并拥有跨来源 `TimelineMaterial` 与 `selectThreadTimelineMaterials`。实际 production 投影由 `transcriptState` 直接消费 thread runtime actions，UI 直接读取 transcript selectors。
...
- **实施结果：** B07 已完整删除 `snapshotReplay` / `liveEventHandling` 四个 source/test 文件，并移除 `App.browser.test.tsx` 中材料层 imports 与专属断言；production replay classification、thread runtime、transcript projection 与 Committed Transcript UI 均保留不变。本地提交为 `bdc73c634`（`Remove obsolete live event and snapshot replay materialization`）。
- **已完成实施验证：** type-check 通过；unit `30` files / `300` tests、Browser Mode `24` files / `222` tests 通过；限定 oxfmt、oxlint、ESLint、diff check 与 timeline-material / `runtimeFromAttach` 零残留搜索通过。未运行 build、e2e、protocol generation 或 Rust，未操作远程。
...
- **报告建议：** 保留为 `RA-04-001`、状态“已实施（B07）”、优先级 P2；历史审计结论继续作为删除依据，不再为已移除的闲置管道建立中立 timeline 抽象。
