...
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-08-projection-delta-redux-action-frequency.md:9`: 原 issue 假设每个 transient delta 触发 Redux action。
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-08-projection-delta-redux-action-frequency.md:29`: 原 issue 把 Redux action 投递、Immer reducer 写入和 store subscription 绑定到 delta 频率。
...
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-09-projection-delta-transient-text-concat.md:9`: issue 指向直接追加 `transientText` 的实现形态。
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-09-projection-delta-transient-text-concat.md:17`: issue 说明 JS 字符串不可变导致长回答小 delta 高频追加时可能接近 `O(n^2)`。
...
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-10-live-slot-selector-cache-invalidation.md:9`: 旧 issue 指向 `selectCachedLiveItemsForTurn` 使用 `liveTurn.revision`、`slotKeys`、`slotRevisions` 判断缓存复用。
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-10-live-slot-selector-cache-invalidation.md:15`: 旧 issue 假设每个 projection delta 执行 `slot.revision += 1`。
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-10-live-slot-selector-cache-invalidation.md:21`: 旧 issue 判断每个 delta 会让 live item view cache 失效并重新 materialize `TranscriptRenderableLiveItem[]`。
...
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-03-item-started-dirties-transcript-state.md:17`: issue 明确现状已更新：当前 `itemStarted` 会创建 `turnId + itemId` keyed live slot。
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-03-item-started-dirties-transcript-state.md:22`: issue 指出剩余窄边界：已有 live slot 时，不同 `commitId` 的重复 `itemStarted` 仍会先记录 applied event。
- `docs/superpowers/issues/2026/06/23/2026-06-23-01-codex-gui-frontend-performance-hot-paths/2026-06-23-03-item-started-dirties-transcript-state.md:40`: issue 明确首次 `itemStarted` dirty `transcriptState` 属于预期 live-state 写入。
