...
- 地址发现不依赖 `tailscale` CLI 或其它 VPN 专用命令。
...
- VPN 使用 active 接口上的首选 VPN / overlay 地址。
- 如果某一类不存在，则不展示该类。
...
这让同一个端口同时服务 Local、LAN 和 IPv4 VPN URL。IPv6 ULA 是 VPN 首选规则的 fallback 候选，但只有当实现能在同一端口服务 IPv6 时才允许输出 IPv6 URL；否则不得输出不可用的 ULA URL。
...
1. 优先选择 `100.64.0.0/10` IPv4。
2. 没有 CGNAT IPv4 时，选择 `fc00::/7` IPv6 ULA，但前提是 GUI host 能在同一端口服务 IPv6。
3. IPv6 URL 必须使用 bracket 格式：
...
- 枚举或接收 advertised host candidates。
- 生成 `GuiLaunchUrls`。
...
- 如果地址枚举失败，回退到只展示 Local，但 host 仍可绑定成功时继续运行。
...
- CGNAT IPv4 优先于 ULA IPv6。
- IPv6 URL 只在同端口 IPv6 服务可用时输出，并使用 bracket 格式。
