...
旧文档 `docs/superpowers/specs/2026/07/15/2026-07-15-codex-gui-transport-session-handshake-owner-design.md` 保留为历史记录，用于说明当时的证据与决策过程；其中关于固定 request ID、手写 protocol guards、raw generic request client 和 runtime trust boundary 的假设已经过时，不得直接执行，也不得作为后续实施计划的接口依据。
