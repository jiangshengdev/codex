...
现有 `docs/superpowers/specs/2026/07/09/2026-07-09-codex-gui-system-performance-check-design.md` 已建立 GUI-wide 微切片、分报告和总报告的审计组织方式，可以作为结构先例。但性能审计的复杂度定义、优先级和 finding 状态不适用于本轮重构审计。

现有 `docs/superpowers/specs/2026/07/11/2026-07-11-codex-gui-transcript-state-split-design.md` 已完整拥有 transcript state 内部 production 模块拆分、测试文件拆分、兼容边界和验证策略。本轮不得将该设计重新包装为新 finding。
...
- 报告编号、最终汇总依赖和交付顺序只定义产出关系，不等于所有调研、写入、审查和整合都必须按编号串行执行；实际调度由微阶段的真实依赖与写入冲突决定。
...
- dependency-ready 指前置证据与门禁已经完成、读取或写入范围已经确定，且不存在共享写入冲突。独立且 dependency-ready 的微阶段优先并行；只有真实证据依赖、顺序门禁或共享写入冲突才能要求串行。
- 报告编号相邻、属于同一调研方向或需要连续复用某个子代理，本身都不构成串行理由。
...
```text
docs/superpowers/reports/2026/07/11/2026-07-11-codex-gui-refactoring-audit/
  00-summary.md
...
`01` 至 `08` 的报告草稿全部完成并记录暂定 evidence owner 后，`09` 才进入 dependency-ready 并允许启动。`09` 负责跨报告去重和唯一 owner 确认；如需修订，必须交回对应报告的单一写入者完成，修订收敛后 `09` 才能完成。此后 `00` 才进入 dependency-ready 并允许启动。这两项是报告集的真实硬依赖。除此之外，`01` 至 `08` 的独立只读调研和不同报告文件的写入不因报告编号或交付顺序而相互阻塞。
...
- 根据依赖图调度当前 dependency-ready 的微阶段，不按 Task 或报告编号机械串行，也不以接近或填满并发上限为目标。
- 允许 `01` 至 `08` 之间没有依赖的只读微阶段跨报告并行；同一报告内没有依赖的调研微阶段也可以并行。
- 对同一真实依赖链，优先复用同一子代理顺序推进；收到结果后先记录该链路的阶段状态和压缩结论，再决定是否进入该链路的下一阶段。
- “先记录结果再进入下一阶段”只约束同一依赖链或共享报告文件上的后续动作，不阻止其他 dependency-ready 的独立微阶段启动或继续。
- 为每份报告指定单一写入者，避免同一文件的并发写入冲突；不同报告文件可由各自写入者并行写入。
- 某份报告正在写入、审查或阶段收敛时，若存在独立 ready 工作且并行协调收益为正，则使用空闲槽推进该工作；不为填满并发上限而启动任务。
- 自行抽查关键定义、构造方、调用方和测试证据。
- 维护 evidence owner、覆盖矩阵、跨报告去重和最终判断。
- 只负责协调、抽查、维护 owner 与依赖以及形成最终判断，不写入任何报告文件。
...
- 每份报告只指定一个写入子代理，该子代理只能修改其独占的报告文件，并且只能按主代理已经审核的结论写入。
- 不同报告文件的写入子代理可以并行工作，但不得跨入其他报告文件或修改 `codex-gui/**`。
- 不运行测试、构建、格式化、profiling、browser automation 或 package scripts。
...
- **共享写入冲突：** 每份报告保持单一写入者；同一报告的写入、修订和阶段整合按共享文件门禁串行，不把该门禁扩展到其他报告文件。
- **跨报告 evidence owner 漂移：** `01` 至 `08` 的草稿先记录暂定 owner 和交界引用；草稿全部完成后才启动 `09` 统一去重和确认唯一 owner。需要修订时由对应报告的单一写入者完成，修订收敛后 `09` 才能完成。
...
- 执行调度以依赖图和当前 ready 微阶段为依据，不按 Task 或报告编号机械串行。
- 并行数量由实际可独立推进的工作决定，不以接近或填满并发上限为目标；真实依赖和共享写入冲突仍受到串行门禁约束。

## 后续边界

本设计确认后，下一轮只能编写并落盘审计执行计划。计划必须固定：

- 报告产出依赖、微阶段依赖图和每份报告的微切片任务。
- 当前可并行的 dependency-ready 微阶段、每个报告文件的单一写入 owner，以及 `01` 至 `08` 草稿全部完成并记录暂定 owner 后 `09` 才允许启动、`09` 去重和修订收敛后 `00` 才允许启动的汇总门禁。
- 每个阶段允许读取的代码、测试、历史文档和 issue 范围。
- 同一依赖链的子代理连续推进规则、主代理抽查点和阶段结果记录方式；不得把子代理复用或结果记录解释为跨报告的全局串行门禁。
